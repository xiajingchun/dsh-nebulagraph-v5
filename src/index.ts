...
export function apply(ctx: Context, config: Config): void {
  ...
  ctx.systemPrompt.section({
    name: 'tool:nebula',
    order: 110,
    text: 'Use nebula_connect to connect to a NebulaGraph v5 server (returns a connectionId), nebula_execute to run GQL statements such as SHOW GRAPHS, SESSION SET graph <name>, and MATCH/GET queries (results come back as structured rows plus an ngql-style table), and nebula_disconnect when done. One server-side session persists per connectionId, so SESSION SET graph applies to later queries on the same connection. To learn a graph\'s schema (node types, edge types, labels, keys, properties), call nebula_schema — it runs SHOW GRAPHS and DESC GRAPH TYPE for you, and the Web Client renders the schema as an interactive G6 graph, so never re-render schema results as extra charts or diagrams (e.g. dsh-ui) in replies; text summaries and plain tables are enough. When a user asks to return a node type or edge type (e.g. "return Star Wars directors and actors"), default the RETURN clause to the complete elements — the pattern\'s node and edge variables (or the path variable) — so the Web Client renders them as an interactive G6 graph; project a specific property (e.g. name) only when the user explicitly asks for it. Only NebulaGraph v5 ISO-GQL syntax (as documented in the gql-query-generator skill) is supported. For writing or migrating GQL queries, load the gql-query-generator skill first. Never ask the user for a password: nebula_connect resolves it from the configured passwordRef (credentials seam / environment) — if a connection needs credentials the user has not configured, say so and point them at the plugin config instead of accepting a password in chat. When nebula_connect fails, do not keep retrying with different arguments: a passwordRef resolution error means the credential is missing (tell the user how to configure it), and a tls: "on" failure means the server has no TLS or an untrusted certificate (report it — changing host/user/port/tls cannot fix it). Never work around a TLS failure by downgrading the transport: when plugin config enforces tls: "on" or "off", do not pass a different tls argument to relax it (the tool rejects conflicting overrides) and do not suggest the user relax the policy — a TLS enforcement failure is a server configuration issue to report, not a connection to force. One failed attempt per distinct cause is enough; report the error and ask the user how to proceed.',
  })
